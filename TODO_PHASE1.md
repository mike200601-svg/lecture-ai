...
### T8.4 性能实测与档位确定 `[~]` ← **阻塞：需要真实录音 + 模型下载**

> `scripts/bench_asr.py` 已实现。tiny 档已测得 7.26x 实时（合成语音）。
> 127.0.0.1:10808 代理可连通 HuggingFace API 与 CDN，但 medium / large-v3-turbo
> 权重下载仍不稳定，目前均为 partial，需要网络条件允许时续传。
