...
- **git**
- **ffmpeg**（第 3 步会装）
- **不需要显卡。** 没有 NVIDIA 卡时走 CPU，medium 模型实测转录耗时约为录音时长的
  **0.44–0.67 倍**（100 分钟的课约 45–65 分钟）
- 硬盘留 3 GB 以上（medium 权重约 1.5 GB；换 large-v3 需要约 3.1 GB，再留一倍余量）
...
#### medium 换 large-v3 值不值：一次实测

`large-v3` 的账面参数确实更好——权重 3.1 GB（medium 1.5 GB）、梅尔滤波器组
128 维（medium 80 维）、支持 100 种语言（medium 99）。但**在这个项目的真实场景里
提升有限，而且慢**。

2026-09-09 用同一段 3 分钟真实课堂录音（量子力学，老师带口音，内容是
「洛伦兹变换下薛定谔方程不协变」），同参数只换模型：

| | medium | large-v3 |
|---|---|---|
| 速度 | 1.59x 实时 | **1.06x 实时**（慢 1.5 倍） |
| 输出字数 | 750 | 732 |
| 专业名词 | 全错 | 仍然全错 |

逐词看，large-v3 确实好一些，但**没有跨过「能用」那条线**：

| 正确写法 | medium | large-v3 |
|---|---|---|
| 薛定谔方程 | 学零课 / 学雷格 / 邪灵梦 | 薛宁格 / 薛伦格 / 斜轮波 |
| 洛伦兹变换 | 罗仁茨 / 罗能辞 | 罗伦茨 / 罗仁慈 |
| 动能部分加势能 | 动人部分讲式能 | **动能部分加势能** ✓ |
| 哈密顿量 | 哈密特量 / 哈密德量 | 哈密等量 / 含义的量 |

结论：**large-v3 在普通中文和常见物理词汇上明显更稳**（「动能/势能」这类它能对，
medium 完全错乱），**但专业名词两个都救不回来**——因为瓶颈不是声学模型，
而是**没有把课程术语表交给 ASR**（见下节）。

所以默认仍是 medium。愿意用一倍时间换更干净的底稿再换 large-v3；
一节 114 分钟的课会从约 65 分钟涨到 2 小时左右，上午的课下午才出稿。

#### 专业名词为什么老是错

ASR 侧的领域引导默认是**关掉**的（`use_hotwords: false`）：实录 A/B 显示，
把整份几十条的术语表塞给 faster-whisper 当 hotwords，会在弱语音和静音段
诱发幻觉（数字电路课冒出「哈密顿量」）。而 `initial_prompt` 这条路在
`condition_on_previous_text: false` 时只对第一个 30 秒窗口有效，等于没用
（faster-whisper 1.2.1 源码里 `prompt_reset_since` 每窗口清空）。

**当前的解法是在下游补**：投喂包的 NOTE_PROMPT 里带上本课术语表，让 GPT 按
清单订正近音错字。这条通道以前是断的——系统认得出是哪门课、也把词表读进了
内存，却谁都没用上。
...
开关都在 `config.yaml` 的 `processing` 段，想退回手动就关掉：

```yaml
processing:
  auto_repair: true            # 转录完成后自动跑 selective repair
  auto_export_package: true    # 有 REPAIRED 后自动生成投喂包
  auto_merge: true             # 自动合并被切断的同一节课（见下）
```

### 录音中途断了怎么办

录音机中途断一次（手滑、来电、存储抖动），一节课就会变成两个文件，watch 会
老老实实建成两个 session：转录分家、投喂包分家、连「这门课的第几次课」的序号
都白占一个。

**默认会自动合并**，你什么都不用做。判定必须同时满足：

- 同一天、同一门课，且落在**同一个课表时段**里（用时段而不是「同一天同一门课」，
  因为一门课一天可能上两次，按天分组会把两节独立的课并成一节）；
- 间隔不超过 `auto_merge_max_gap_minutes`（默认 15 分钟）；
- 匹配不到课表的（`unknown` 课程）没有依据证明属于同一节课，只在间隔 5 分钟
  以内才合。

合并**发生在转录之后**——项目红线是「绝不重跑已经成功的 ASR」，所以只重排
时间轴：后面每段的片段整体偏移 `(本段起始 − 首段起始)`。**音频也会一起拼**，
断口填静音：因为 `repair` 是拿着时间戳回到音频里重转可疑区间的，只并转录不并
音频的话，第二段的时间戳会落在第一段音频范围之外。静音补齐而非首尾相接，是为了
让时间戳继续对齐墙钟，Phase 3 拿板书照片的 EXIF 对时才不会整体偏移。

还有一段**宽限期**：后半段录音常常晚几十分钟才传完（100+ MB），所以在课表时段
结束后的 `merge_grace_minutes`（默认 30 分钟）内，不会先给半节课出投喂包。

自动判定不确定时宁可不合，交给你手动：

```powershell
python -m lecture_ai merge <id_a> <id_b> --dry-run   # 先看会怎么合
python -m lecture_ai merge <id_a> <id_b>
```
...
`NOTE_PROMPT.md` 里带着**本课术语表**（课程词表 + `common.txt`），并明确告诉
GPT：转录来自本地语音识别、专业名词的近音错字很常见，请按清单订正。同时划了
边界——只订正读音相近且语境吻合的，不确定就保留原文并标注，不要猜。

这是本地 ASR 与 GPT 之间唯一的领域知识通道。它一度是断的：系统在 ingest 时
已经按课表认出是哪门课、也把词表读进了内存，但 ASR 侧 hotwords 被实录 A/B
关掉，投喂包这边又从来没把词表交出去，结果几十条术语读进内存后谁都没用上
（量子力学一节实测：65 条只在转录里命中 2 条）。
...
面板顶部常驻一块状态区，每 5 秒刷新（页面切到后台就停）：

- **watch 在不在跑**（PID、心跳多久没动）。探活看的是锁文件 mtime，一次 stat，
  不起子进程 —— 面板高频轮询，不能为了探活反复 fork。
- **正在转录的课**：进度条、百分比、已转 / 总时长、按实测倍速估的剩余时间。
  进度超过 3 分钟没更新会标红「疑似中断」并提示 `retry`，不会挂着一个假进度骗人。
- **排队等转录的课**，以及**最近 12 行日志**（折叠状态跨刷新保留）。

再往上是**告警区**，一切正常时整块不出现，出现即代表要人管：

| 告警 | 触发条件 |
|---|---|
| watch 没在运行 | 新录音不会被自动处理 |
| 同步设备断开 | 录音可能还堵在手机那一端 |
| 长时间没有新录音 | 超过 `monitoring.sync_stale_hours`（默认 6 小时） |
| 上次是异常关机重启 | 读 Windows 事件日志 Kernel-Power 41，带 30 天内崩溃次数 |

这块是被两次真实事故逼出来的：一次是 ffmpeg 每 15 秒弹窗、面板上完全看不见；
一次是手机 Syncthing 核心假死（App 显示「运行中」、端口却不监听）叠加电脑蓝屏
重启，直到中午才发现上午两节课的录音一直躺在手机里。**共同点都不是修不好，
而是没人知道它坏了。**

探测本身有三条硬约束：绝不抛异常（失败只少显示一块，不能让面板打不开）、
昂贵探测一律带缓存（读事件日志约 1 秒，而面板 5 秒轮询一次）、子进程一律带
`CREATE_NO_WINDOW`（`serve` 可能被 pythonw 拉起，否则每次探测闪一个黑框）。
